...
def main():
    ...
    for year in range(args.first_year, args.last_year + 1):
        models[year] = load_model(
            os.path.join(args.embeddings_dir, f"{year}.{args.embeddings_extension}")
        )

    df = pd.read_csv(args.dataset)
    X_train = (
        (row.WORD, models[row.BASE_YEAR], models[row.BASE_YEAR + 1])
        for row in df.itertuples()
    )
    y_train = df.GROUND_TRUTH
    y_train = [0 if el == 0 or el == 1 else 1 for el in y_train]
